...
//Mplayer Declarations Are Over . Now we move to ChronOS Declarations
...
//the next priority task declarations come from utils.h
#define MAIN_PRIO           98
...
#define CHRONOS_PERIOD      10000
#define CHRONOS_DEADLINE    10000 
 #define BILLION             1000000000
static void find_job_deadline(int job, long timeliness, struct timespec *start_time, struct timespec *deadline)
{
    unsigned long long nsec, carry;
    unsigned long offset = timeliness;
...
//==================== START PLAYING =======================
        ...
        // #if only if conditons met then give to compiler
        period.tv_nsec = CHRONOS_PERIOD * 1000;
        chronos_frames = 0;
        unsigned long long diff;
        struct timeval delay,start,completion;
        ...
        while (!mpctx->eof) {
           ...
           // printf("%d frames dropped",drop_frame_cnt);
            if (dvd_last_chapter > 0) {
                int cur_chapter = demuxer_get_current_chapter(mpctx->demuxer);
                if (cur_chapter != -1 && cur_chapter + 1 > dvd_last_chapter)
                    goto goto_next_file;
...
//================= Keyboard events, SEEKing ====================
    ...
    int n_drop = 0;
    unsigned long long now = (completion.tv_sec * 1000000) + completion.tv_usec;
    i_chr = 0;
    ...
    for (i_chr = 0; i_chr < CHRONOS_MAX_TIMESTAMPS && timestamps[i_chr] != 0; i_chr++)
        chronos_fps++;
        diff = (completion.tv_sec * 1000000) + completion.tv_usec;
        diff -= (start.tv_sec * 1000000) + start.tv_usec;
        delay.tv_usec = CHRONOS_PERIOD - diff;
#if 0
    printf("end %ld %ld (%lld)\n", completion.tv_sec, completion.tv_usec, diff);
    printf("frame %d: %lld us %d fps %ld delay %d\n", chronos_frames, diff, 0, delay.tv_usec, delay.tv_usec <= 0);
#endif
    if (delay.tv_usec > 0)
